...
if (localStorage.getItem("agreeToCC")) {
  ...
  const setupShaders = () => {
    ...
    let compositeVertShader = createShader(
      cameraCtx,
      multiplyVert,
      cameraCtx.VERTEX_SHADER
    );
    let compositeFragShader = createShader(
      cameraCtx,
      multiplyFrag,
      cameraCtx.FRAGMENT_SHADER
    );
    ...
    if (
      !cameraCtx.getProgramParameter(compositeProgram, cameraCtx.LINK_STATUS)
    ) {
      var info = cameraCtx.getProgramInfoLog(compositeProgram);
